...
if __name__ == '__main__':
    ...
    if args.modality == 'synth' or args.modality == 'synth_trans':
    ...
    elif args.modality in ['roc', 'roc-aug', 'book', 'simple-wiki', 'e2e-tgt', 'e2e', 'yelp', 'commonGen', 'commonGen-aug']:
        ...
        Model_FILE = f"diff_{args.modality}{0}_{0}_{exp_m}{args.in_channel}_{args.model_arch}_lr{args.lr}" \
                     f"_{args.diff_steps}_{args.noise_schedule}_{args.loss_type}_h{args.hidden_size}" \
                     f"_s{args.num_res_blocks}_sd{args.seed}"
    elif args.modality in ['image', 'permuted_image']:
        Model_FILE = f"diff_{args.modality}{0}_{0}_{exp_m}{args.in_channel}_{args.model_arch}_lr{args.lr}" \
                     f"_{args.diff_steps}_{args.noise_schedule}_{args.loss_type}_h{args.hidden_size}" \
                     f"_s{args.num_res_blocks}_sd{args.seed}"
    ...
    # 确保路径正确
    train_script_path = "/content/Diffusion-LM/improved-diffusion/scripts/train.py"
    ...
    if args.submit == 'no':
        ...
        if args.use_big == 'no':
            full_command = f"nlprun  -g 1 -n {Model_FILE} -x jagupard10,jagupard11,jagupard12,jagupard13,jagupard14,jagupard15,jagupard16,jagupard17,jagupard20 '{COMMANDLINE}'"
        else:
            full_command = f"nlprun  -g 1 -n {Model_FILE} -x " \
                           ...
                           "jagupard24,jagupard25 '{COMMANDLINE}'"
